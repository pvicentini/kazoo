...
validate_document_node(Context, Id, ?HTTP_GET, Node) ->
    Config =
        case kz_term:is_true(cb_context:req_value(Context, <<"with_defaults">>, false)) of
            true -> kapps_config_doc:stored_node(Id, Node);
            false -> kz_json:get_value(Node, kapps_config_doc:get_config(Id), kz_json:new())
        end,
    crossbar_doc:handle_datamgr_success(set_id(Id, Node, Config), Context);
...
post(Context, Id, Node) ->
    ...
    case cb_context:resp_status(Ctx) of
        success ->
            cb_context:set_resp_data(Ctx, set_id(<<Id/binary, "/", Node/binary>>, kz_json:get_value(Node, cb_context:doc(Ctx))));
        _ -> Ctx
    end.
...
set_id(Id, Node, JObj) ->
    kz_doc:set_id(JObj, <<Id/binary, "/", Node/binary>>).
